...
class TestPostToolUseTracker:
    ...
    def test_context_reminder_for_cli_subsystem(self) -> None:
        """Editing a CLI source file triggers a context staleness reminder."""
        reminder = post_tool_use_tracker.check_context_staleness(
            "src/command/install.rs"
        )
        assert reminder is not None
        assert "CLI" in reminder
...
class TestPostToolUseTrackerMarkdownInfra:
    ...
    def test_user_docs_have_no_config_reminder(self) -> None:
        """User docs are not AI config — no reminder entry should match."""
    ...
    def test_is_ai_config_markdown_user_doc(self) -> None:
        """User docs are not AI config markdown."""
        assert post_tool_use_tracker._is_ai_config_markdown(
            "docs/foo.md"
        ) is False
