...
Path-scoped rule (auto-loads on source-work surfaces: `src/**`, `test/**`, `.claude/**`, `Cargo.toml`, `Cargo.lock`). Referenced from [workflow-intent.md](./workflow-intent.md) when work classified as refactoring. Enforces Two Hats Rule: change structure, no behavior change.
