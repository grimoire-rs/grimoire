...
**Carve-out:** a class-2 render derived solely from the artifact's own
frontmatter — no upstream key, surface or version behind it — is not
tier-gated, because there is nothing to re-verify and so nothing the tier
buys (`render::scope_notice`, the dropped-`paths:` prose, is the case).
A class-2 render that *tracks* a vendor surface still is.
...
| Rule scoping | OpenCode | `Degraded` — `paths:` dropped + warning, the scope restated as a prose notice in the body (class-2, frontmatter-only) | no per-file rule scoping documented; `instructions` globs are file-selection, not activation conditions | promote to `Native` and drop the notice + warning when a per-file activation key ships |
...
| `claudeMdExcludes` | Claude | registers one element per support-dir rule (`**/.claude/rules/<name>/**`; absolute at global scope) so grim's own verbatim support-dir copy stops auto-loading as unconditional context — `src/install/claude_config.rs` | *verified 2026-08-25 against CLI 2.1.245.* Shipped and documented ([claude memory docs](https://code.claude.com/docs/en/memory)): configurable at **any** settings layer (user, project, local, managed policy), **arrays merge across layers**, patterns matched against **absolute file paths** with glob syntax; the docs' own example excludes a rules directory. Managed-policy CLAUDE.md cannot be excluded — irrelevant here, grim never writes one. Note the docs steer humans to `settings.local.json`; grim deliberately writes the committed `settings.json` (the rule files it excludes are committed too) | **re-verify before touching `claude_config.rs`.** If the key is renamed or exclusion stops suppressing recursive `rules/` discovery, the support-dir over-load (grimoire-rs/grimoire#102) silently returns — the fallback is the rejected alternative, relocating the support dir out of `rules/` for Claude |
...
All rows `verified 2026-08-17` (the Hook kind shipped arming for three of
eighteen clients). Sources: the per-vendor hook survey under
`.agents/research/`, `hooks_vendor_reports/*.md`, and the live vendor
verification spike whose findings were **executed**, not read.

Fifteen clients decline Hook, and the declines are **not one condition** —
grouping them would hide the only two rows that can flip without an upstream
release.

| Capability | Vendor | Current grim behavior | Upstream status | Action when shipped |
|---|---|---|---|---|
| Hook kind | Cursor, Gemini, Droid, Antigravity, Junie | declined | **grim-side schedule, not an upstream gap.** Each splices shell-command hooks into a shared config file — the same JSON-splice machinery grim already ships for MCP. Nearest flip candidates in the batch | wire `hook_registration` + a matcher-dialect row per vendor; enable in `kind_support` |
| Hook kind | Kiro, Goose, Cline | declined | **grim-side schedule.** Each owns a hooks file or directory of its own, so grim would own the file outright (the Codex/Copilot `hooks.json` shape) rather than splicing a shared one | same as above; whole-file ownership is the simpler of the two shapes |
| Hook kind | OpenCode, Amp, Kilo, OpenClaw | declined | **structurally further out.** Each takes a JavaScript/TypeScript **plugin module**, which grim would have to *generate* rather than register — a code generator, not a config write | do not treat as near-term; needs its own design decision first |
| Hook kind | Zed, Warp | declined | **no upstream surface at all.** Zed's agent runs over ACP with no lifecycle-hook mechanism; Warp documents none. Cannot be reversed by a grim release alone | re-verify only after the vendor ships a hook surface |
| Hook kind | `agents` (vendor-neutral) | declined | **not a client.** A shared directory, not a product — no process reads a registration placed there. Same reason it declines rules, agents and MCP | never — this is correct by construction |
| Blocking verdict on `PostToolUse` | Copilot | declined for a `gatekeeper` on the after-a-tool-ran event — warn, skip, no registration; every other event/tier arms normally | *verified 2026-08-17.* Copilot's own event table accepts no blocking verdict there, where Claude and Codex both do. Holds for **every** matcher form, exact name and match-all alike — which is what makes it degrade the whole cell to `◐` rather than being a per-hook fact | promote the Hook cell to `✓` when upstream admits a deny response on that event; documented as `docs/src/clients.md` `{#gap-copilot-hooks}` |
| `mutator` tier | Copilot | **supported, not declined** — ADR Open Question 2 is *settled by execution*, in the PascalCase dialect. Do **not** re-file this as a pending decline | *verified 2026-08-17.* Two live residuals, both accepted and disclosed rather than open: (a) `mutator` is refused **per tool, not per event** — decision K declines it for a matcher that could select `Bash`, enforced in `Vendor::hook_registration`; (b) with a mutation applied, Copilot's transcript still **displays the original command while executing the rewritten one**, an observed vendor behaviour that mutator control 5 (S-016) exists to compensate for | nothing to flip. Re-verify (b) against a newer CLI — if the transcript is fixed upstream, S-016's compensation can be reconsidered. The field name lives in `RESPONSE_PROJECTION`; read it there, never restate it |
| `preToolUse` fails **closed** | Copilot | the registered command keeps its `[ -x "$L" ] \|\| exit 0` guard, and Copilot's exec-form `exec`/`args` field is **never** used | *verified 2026-08-17.* Copilot is the only one of the three v1 clients that blocks on a non-zero hook exit, so the guard is mandatory rather than tidy: exec form means no shell, means no guard, and a missing launcher becomes a spawn failure that denies the tool call | none — do not "simplify" to exec form. If exec form is ever adopted, the launcher-absent case must be made non-denying first |
| Hooks run through `$SHELL -lc` | Codex | grim's registered command is a **POSIX** one-liner (`[ -f … ] && [ -x … ] \|\| exit 0`, `s=$?`, `case`) | *found by execution.* Codex invokes hooks via `$SHELL -lc`, so a user whose `$SHELL` is `fish` or `nushell` **cannot execute the guard at all** — the added `s=$?`/`case` forms make the string no more portable. Not hypothetical, and not fixable by more POSIX | emit a shell-neutral registration (or an exec-form argv) for Codex; until then, document the `$SHELL` constraint rather than widening the one-liner |
| Client-side hook approval | Codex | reported as `state: untrusted`, cause `client-trust-pending` | *verified by execution.* Codex requires an interactive `/hooks` approval and skips an unapproved hook **silently** — which is exactly why grim reports the pending state rather than claiming the hook is armed | none — the silence is upstream's; keep the distinct cause so it never collapses into a generic `not-armed` |
| Project-scope hooks | Codex, Copilot | **global scope only** | *not feasible in v1, and the reason is structural rather than scheduling.* A committed registration makes the executed **binary path** environment-derived (CWE-426), and every grim control is downstream of it. Copilot's `{{project_dir}}` makes `--root` portable but not the launcher path; Codex passes no data into the command at all, leaving only `$PWD` — the session cwd, mutable mid-session | revisit only if a client offers a portable way to name the launcher that a repository cannot influence |
