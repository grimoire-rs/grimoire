...
| **N4** | **A user deliberately bypassing a gate they were shown.** `--force` on an untracked destination, `--trust-hooks` in their own CI, approving a hook they did not read. | Consent means the user may consent. The obligation is that the prompt be **honest and legible**, not that it be unbypassable. |
...
2. **I2 — Identity is content, and it is re-checked at the moment of use.** Approval and trust key
   ...
   > **Hooks carry a recorded, bounded exception to the "never on a name" clause, and it is worth
   > reading before filing it as a Block.** Two trades were already recorded here: A3 dropped the
   > exec-time digest re-check (*"a recorded trade against I2, resolved as I5"*), and A2's
   > registry-scoped `trust_hooks` keyed the *authorization* decision on a registry locator — a
   > name. As of **2026-08-28** that registry key is replaced by a **workspace path** key: the
   > consent record at `$GRIM_HOME/hooks/consent/<workspace-key>.json`
   > (`adr_hook_workspace_consent.md`). **This changes which name is keyed on; it does not widen
   > the trade.**
   >
   > What holds it in bounds is that the two halves answer different questions and neither
   > substitutes for the other:
   >
   > - **The digest half still holds, unchanged.** *Which bytes* an artifact is remains the lock's
   >   pinned manifest digest, verified at install time against the registry's answer. T1 and T2
   >   are answered there, exactly as `adr_artifact_trust_model.md` decisions 3 and 4 say, and
   >   consent never stands in for a pin.
   > - **The name half answers a question that has no content to key on.** *May this checkout arm
   >   hooks on this machine at all* is not a property of any artifact — a workspace is not
   >   content and has no digest. Keying it on content is not a stricter option here; it is the
   >   documented bug. `direnv` shipped content-only trust and had to add the path, because an
   >   approved `.envrc` copied into a hostile directory executed (`direnv/direnv#83`).
   >
   > The residual this leaves is named rather than engineered against: a publisher who owns a
   > repository a workspace already consented to can ship a **new version** of that hook without a
   > re-prompt, because the consented set carries no tag and no digest. That is **T1**, its answer
   > is the pin, and its visibility is `git diff` on the lock plus `grim status` — **I5**,
   > evidence, not prevention. State it that way; do not describe consent as preventing it.
...
- **Reviewing a design:** check each invariant that the change touches. A change that moves
  ...
  it is mitigated — **unless** it is the hook-consent exception recorded under I2 above, which is
  a decided trade with its residual named. Re-deriving it as a fresh Block is the churn this file
  exists to stop; disagreeing with it is an ADR amendment, not a finding.
...
- `.agents/adr/adr_hook_workspace_consent.md` — the workspace-consent decision and the I2 trade
  recorded above; `.agents/adr/adr_artifact_trust_model.md` — what integrity buys and what consent
  does not
