...
[Taskfile](https://taskfile.dev) v3 = primary task runner for Grimoire. Root taskfile includes per-subsystem taskfiles + reusable templates under `taskfiles/`. Each subsystem own pipeline, expose contract back to root.
...
| `taskfiles/rust.taskfile.yml` | Rust subsystem | `rust:` -- format, clippy, build, test:unit |
...
| Include-with-vars templates | reusable taskfiles included with different `vars:`; use `requires:` for mandatory vars |
...
5. **Subsystem env overrides go on task, not include.** When the root sets a global `env:`, a subsystem task needing a different value overrides it with a per-task `env:`.
...
generate:
  cmds: [cargo run --release -- some-subcommand > {{.OUT}}]
  sources: [src/**/*.rs, Cargo.lock]
  status: [test -f {{.OUT}}]
...
build:
  ...
  sources: [src/**/*.rs, Cargo.lock]
  status: [test target/release/grim -nt Cargo.lock]
  method: timestamp
```
...
Set `dir:` on include block when all tasks should run relative to sub-taskfile's directory (`.claude/`, `test/`). Override per-task only when needed.
...
## Grimoire-Specific Task Contracts

- **Generation tasks** that run the compiled binary should depend on it via `deps: [build]` + `sources: [target/release/grim]`, NOT on Rust source file lists. Cargo already tracks source deps — duplicating in the Taskfile = maintenance overhead.
