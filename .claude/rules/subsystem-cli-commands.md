...
| `grim config registry add\|set\|rm\|use\|show\|list\|fields` | Registry lifecycle for `[[registries]]` entries: `add <alias> --oci\|--index <url> [--include <glob>]… [--exclude <glob>]… [--default]`, `set <alias>` (edit in place — same flags as `add` plus `--clear-include`/`--clear-exclude`; applies only the flags given, keeps the entry's position, `--oci`/`--index` swaps the kind clearing the other side, `--default` cannot unset; naming no field → 64), `rm <alias>`, `use <alias>` (at-most-one-default, clears prior), `show <alias>`, `list`. Alias not found (`set`/`rm`/`use`/`show`) or dup on `add` → 64. `--include`/`--exclude` are repeatable and **never comma-split** (a comma is glob alternation) — `add`/`set` are the only CLI paths writing a multi-pattern browse filter; `config set registry.<alias>.include` stores exactly one pattern, and `get` comma-joins for display and is not round-trippable. Clearing a list has two supported routes, both silent at any list length and both writing the emptied list as an absent key: `registry set --clear-include`/`--clear-exclude` (conflicts with its own list flag → 64) and `config unset registry.<alias>.include`. `registry set`'s write report carries the always-present `fields` array — one element per field written, in `RegistryField::ALL` order, `{field, action: set, value}` or `{field, action: cleared}` (no `value` key); `[]` on every other write verb. `show`/`list` carry always-present `include`/`exclude` arrays (`[]` unfiltered; plain cell is counts, `—` unfiltered). `fields` lists the 5 addressable per-registry field names (`oci`, `index`, `default`, `include`, `exclude` — **append-only**, positions frozen) and their static type/title/description metadata — no `Context`, no scope resolve, no lock, works with no `grimoire.toml` present (unlike every other `registry` verb) |
