...
| `grim status` | Report each declared artifact's state (installed, outdated, modified, …) with bundle provenance; JSON items carry always-present `clients_missing`/`clients_extra` (configured-vs-recorded client drift, local only) and always-present `outputs_pending` (`[{client, path}]`, materialization drift: what `grim install` would write now that the record does not cover — new client, deleted file, layout move; reported under autodetect too, since it asks what install would do rather than what was configured; never moves `state`, never affects the exit code; derived from `install::expected_outputs`, the same seam the installer's no-op check uses). `--check` (network, opt-in) adds a live catalog load populating `deprecated`/`replaced_by`, plus a fresh per-artifact re-resolution populating `update_available`; top-level `checked` gates all three — `false` forces them `null` on every item |
