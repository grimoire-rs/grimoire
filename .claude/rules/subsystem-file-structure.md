...
because *the registration is never recorded at all*, plus a trust gate no
other kind has any precedent for.
...
full set is `bin`, `dispatch.json`, `dispatch.json.lock`, `payload`, `root-key`,
and it is deliberately **not** restated as a count anywhere: this list fell one
behind the layout twice (`root-key`, then the lock sidecar), each time because a
prose enumeration was cheaper to leave alone than to check. The array in
