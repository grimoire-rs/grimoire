...
Shared branch-and-commit hygiene for Grimoire. Used by `/commit` skill (working phase) and `/finalize` skill (rebasing phase). Catalog-only: referenced on demand, not auto-loaded via path glob — nothing in repo "a git file".
...
- **Never `Co-Authored-By`** in commit messages. Grimoire convention.
...
2. **Format only new code**: Do NOT run `rustfmt`/`cargo fmt` on entire file — only format lines you introduced. Upstream may use different style (e.g., 100-char width vs Grimoire 120-char). Reformatting bloats diffs and blocks upstream PRs.
