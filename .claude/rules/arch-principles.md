...
| [adr_announce_fork.md](../artifacts/adr_announce_fork.md) | `grim publish --announce` auto-forks the index repo when the token has no push access, turning today's exit-69 push failure into a cross-repository PR/MR. `[announce] fork` is a tri-state `ForkPolicy` — `auto` (default) forks ONLY on a certain `Some(false)` push probe, `never` always pushes upstream, `always` forks even WITH push access (bypasses that one early return, nothing else); the legacy bool is a permanent alias (`true`=auto, `false`=never) via a hand-written `ForkSetting` `Deserialize` (not `untagged`, so a typo gets serde's `unknown variant` list). `ensure_fork` → `Result<Option<ForkTarget>, AnnounceError>`: ambiguous/absent probe degrades to the upstream push, never worse than exit 69; `Ok(None)` = degrade, `Err(Fork)` only once a fork is required and cannot be created/verified/readied. Self-fork guard is policy-independent and explicit on BOTH forges (`force` made the GitLab path reachable, where a self-owned upstream would 409 then fail the reuse hunt into exit 69). Parent-verification guard on BOTH create and reuse (GitHub `parent.full_name` / GitLab `forked_from_project.id` must match upstream — refuses a same-named stranger repo); `ForkTarget` fields read from the API response body, never `{login}/{basename}` (renamed forks). Session decisions: GitLab existing-fork reuse becomes identity-based (enumerate `GET /projects/:id/forks`, match source id + authenticated namespace, poll by project id) replacing the basename guess that failed for renamed/group/concurrent forks; hardened readiness (per-attempt timeout + wall-clock deadline + GitLab `import_status=="failed"` fast-fail + progress line + a bounded GitHub push-retry for the metadata-before-git-objects gap). Security hardening (landing as follow-ups): https-validate `push_url` before use as a git remote, no-redirect forge client so auth headers can't leak cross-host, `git push` restricts transports + `--` end-of-options, exhaustive `ForgeKind` matches in `authorize`/`resolve`. Orphan-fork-on-partial-failure is benign + adopted (`created=false`) on re-run; deterministic branch + PR/MR reuse keep it idempotent. Principle 9 additive: `announce.fork` always-present-null, `AnnounceError` `#[non_exhaustive]` (`Fork` → exit 69). Amends adr_grim_publish D6 report shape |
