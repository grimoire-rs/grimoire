...
| [adr_oci_empty_config_compat.md](../artifacts/adr_oci_empty_config_compat.md) | Type artifacts with OCI `artifactType` + the OCI empty config descriptor (GitLab-allowlist-safe) + a `com.grimoire.kind` annotation fallback; 3-tier kind read (artifactType → legacy config mediaType → annotation); supersedes adr_oci_artifact_type |
