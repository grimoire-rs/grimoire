---
name: commit
description: Use when the user says "commit", "/commit", "save progress", or asks to land working changes on a feature or worktree branch. Grimoire working-phase posture — minimises commit count, can amend rolling Checkpoints, offers a one-time PR prompt. For pre-merge cleanup, use `/finalize`.
user-invocable: true
...
Commit working tree during active dev on feature/worktree branch. **Working phase** half of Grimoire two-phase branch workflow — rebasing-phase cleanup for `main` lives in `/finalize`.
...
Optional free-text arg. Treat as intent hint (e.g., `fix grimoire install symlink bug`), not full message — still draft final message. Flags:
...
- `git config --get branch.<current>.grim-skip-pr-prompt` — PR prompt already answered?
...
- **Never** `Co-Authored-By` (Grimoire convention)
...
- `git config --get branch.<current>.grim-skip-pr-prompt` returns `true`
...
2. **Stay on current branch** — commit here. Record: `git config branch.<current>.grim-skip-pr-prompt true`. Never ask again unless `/commit --pr`
3. **Cancel** — abort
