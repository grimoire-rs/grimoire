...
Per-tier defaults:
- low → `haiku` (→ `sonnet` when structural markers from `classify.md:48-61` fire: auth/crypto/signing, new `Cargo.toml` deps, `deny.toml`, core `src/**` changes, public API breakage)
- high → `sonnet`
...
| Value | Effect |
|---|---|
| `haiku` | `worker-doc-reviewer` with model=haiku. Triggered when diff touches ≤2 doc files AND does not touch the primary user guide under `docs/`. |
