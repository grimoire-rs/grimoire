...
Role: test strategy, writing, validation for Grimoire.
...
- **Isolated** — per-test GRIM_HOME, unique repo names, no shared state
...
- `.claude/rules/subsystem-tests.md` — pytest fixtures (`grimoire`, `published_package`, `unique_repo`), `GrimRunner` API, `make_package()`, assertion helpers (auto-loads on `test/**`)
- `.claude/rules/quality-core.md` + `quality-python.md` — test quality, async patterns
- `.claude/rules/quality-rust.md` — unit test patterns for the crate
