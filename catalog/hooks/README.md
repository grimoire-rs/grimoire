...
### 3. Turn hooks on, and grant trust
...
# Gate 2 — trust for the registry these came from, in GLOBAL config.
grim config registry add examples --oci localhost:5000/grim-examples --global
grim config set registry.examples.trust_hooks true --global
```

Trust must be granted **globally**. A repository file may restrict hook
trust, never grant it — otherwise a repo you cloned could vouch for itself.
For a one-off run without persisting anything, `grim install --allow-hooks`
grants trust for that invocation only.
...
Revoke the trust grant as well:

```sh
grim config set registry.examples.trust_hooks false --global
```
