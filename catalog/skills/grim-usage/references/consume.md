...
same gates a directly declared hook faces — the feature flag, then the
**registry** trust question answered against the member's *own* source, not
the bundle's, so trusting the bundle's registry never silently grants a
member pinned from a different one. Removing the bundle removes the hook,
unless you also declared it directly.
...
| 2 | Is this artifact's registry trusted for hooks? | the per-registry `trust_hooks` field | see below |
| 2 | …or did this one invocation get an explicit pass? | `--allow-hooks` on `grim install` / `update` / `add` | off |
| 2 | …or did you answer the prompt? | asked once per registry, on a TTY only | — |

`--allow-hooks` is per-invocation and **never persisted**, and it does *not*
turn the feature on — the flag above must already be set. It is a **flag
only: there is no `GRIM_ALLOW_HOOKS`**, and that absence is deliberate. The
...
would let a cloned repository grant itself trust.
...
Enabling the feature is not consent to run any particular hook. The
per-registry `trust_hooks` field is a tri-state with a deliberate asymmetry.

In **global** config an unset value means trusted *for most entries*, because
configuring a registry there is itself the trust act — but two shapes never
grant implicitly, and both are easy to hit:

- a **bare host** (`oci = "ghcr.io"`), which is the whole shared multi-tenant
  registry: an implicit grant there would consent to every publisher on it.
  Add the namespace (`ghcr.io/acme`) or set `trust_hooks = true` explicitly.
- an entry declaring **`insecure = true`**, because plain HTTP makes the first
  resolution attacker-influenceable on the wire and no digest pin can rescue
  that. Loopback is the one exemption.

An `index` entry never grants either. An explicit `false` opts out, and beats
every grant in either scope.

In **project** config the key may only *restrict* — a file that travels with a
repository never grants trust, so `false` still opts out while neither unset
nor `true` grants anything. Inspect it with
`grim config get registry.<alias>.trust_hooks`, which unlike the feature flag
does print an authored `false`.
