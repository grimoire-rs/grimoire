...
grim config get registry.<alias>.trust_hooks      # exit 1 = unset; prints false = opted out
```
...
1. **The feature flag.** `options.experimental.hooks` is `false` unless
   ...
   "off" looks like**. `trust_hooks` behaves differently on purpose — it is a
   tri-state and an authored `false` is kept, because silently dropping an
   opt-out would re-arm a registry the user deliberately excluded.
2. **The registry's `trust_hooks`.** A separate question from the flag, and an
   unset value in global config does **not** simply mean trusted. A *global*
   `[[registries]]` entry grants implicitly only when all three hold:
   - it is an `oci` entry (an `index` entry never grants);
   - it is **namespaced** (`ghcr.io/acme`, not the bare host `ghcr.io` — a bare
     host is the whole shared multi-tenant registry, so granting it would
     consent to every publisher on it);
   - it does not declare `insecure = true` (plain HTTP makes the first
     resolution attacker-influenceable on the wire, so the digest pin cannot
     rescue it). Loopback is the one exemption.

   Anything else prompts on a TTY and reports `registry-not-trusted` otherwise.
   In *project* config the key can only restrict, so a `trust_hooks = false`
   that travelled in with a cloned repository opts the registry out and no
   project-level edit can grant it back. Move the grant to global config —
   adding the **namespace** to the locator is often the whole fix.
