...
1. **Catalog metadata authored** — `summary` present (search shows it
   ...
   frontmatter; mcp/bundle → top-level TOML. **A hook is the exception with
   nowhere to put any of it** — `hook.toml` accepts only `schema`, `name`,
   `description`, and `[[hooks]]`, so every catalog key is a hard parse
   error (65) and `description` carries the whole blurb. If this release retires a
