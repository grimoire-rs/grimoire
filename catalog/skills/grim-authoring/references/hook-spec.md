...
- Enabling the flag is not consent to run your hook: a consumer's
  per-registry `trust_hooks` field is a second, independent question, and a
  project-level config can only ever *restrict* it, never grant it.
...
- **A hook can be a bundle member.** Add a `[hooks]` table to the bundle
  ...
  **Membership declares; it never arms.** A bundle-delivered hook lands in
  ...
  hook faces — the feature flag first, then the member registry's own
  `trust_hooks` grant. There is no per-hook approval anywhere in the design:
  consent is asked once per registry and covers every hook from it.
  Packaging a hook inside a bundle is not a way around that gate, and the
  trust decision keys on the member's own source, so approving a bundle from
  one registry never silently approves a member pinned from another.
