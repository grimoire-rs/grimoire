...
| `CLAUDE_CONFIG_DIR`, `COPILOT_HOME`, `OPENCODE_CONFIG_DIR`, `CODEX_HOME` | Vendor config-dir overrides (honored read-only). Global-scope installs follow them: `CLAUDE_CONFIG_DIR` replaces `~/.claude` (skills, rules, and agents) and relocates the global MCP registration file to `$CLAUDE_CONFIG_DIR/.claude.json`, `COPILOT_HOME` replaces `~/.copilot` (skills and agents), `OPENCODE_CONFIG_DIR` is the preferred install target over the XDG default for OpenCode skills and agents (additive — OpenCode scans both), `CODEX_HOME` replaces `~/.codex` for Codex agents and the MCP `config.toml` (Codex skills follow the cross-vendor `$HOME/.agents/skills` standard, NOT relocated by it). They also drive global-scope client *detection* — a client counts as present when its (possibly overridden) native root exists. Details → subsystem-file-structure.md | (unset) |
