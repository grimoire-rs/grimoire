...
- **⛔ C-022 IS SUPERSEDED IN FULL — 2026-08-28, by [`adr_hook_workspace_consent.md`](../adr/adr_hook_workspace_consent.md). Do not implement anything in this bullet or its amendment box.**
  Registry-scoped trust is replaced by a **per-workspace consent record** at
  `$GRIM_HOME/hooks/consent/<workspace-key>.json`. There is no `trust_hooks`
  field, no `registry.<alias>.trust_hooks` key, and **no config file of any
  scope is an input to an arming decision** — which is why the whole B4
  precedence table, B5's locator matching (path-segment prefix, bare-host,
  `index`), and B7's `Option<bool>` serializer hazard **dissolve** rather than
  moving: their subject ceases to exist. `RegistryField::ALL` returns to the six
  shipped names in their shipped order.
  **W8 / S2-2 are the one part that survives, relocated.** The plain-HTTP rule
  is not deleted: it becomes a standalone transport gate ahead of consent — a
  hook whose pinned registry host is non-loopback and reached over plain HTTP
  does not arm, cause `insecure-transport`. That is *stronger* than the clause
  below, which could be escaped by writing `trust_hooks = true` from a config
  file; the gate cannot be escaped by any file, only by `--trust-hooks` (N4).
  Its loopback assertions in `only_a_loopback_host_is_exempt_from_the_insecure_rule_b3`
  are **moved, not rewritten**.
  B6's `GRIM_ALLOW_HOOKS` disposition is unaffected and already settled: the
  variable does not exist (owner decision 2026-08-17, `24a14bb`), and there is
  no `GRIM_HOOK_CONSENT` either.
  The reason the reversal is free: `git grep trust_hooks origin/main` returns
  nothing. Nothing released ever carried the field, so **Principle 9 does not
  bind** — the one residual is that a branch tester's global config still
  carrying `trust_hooks = true` exits **78** under `deny_unknown_fields`, which
  is accepted with no shim.
...
- **C-023 — non-interactive never blocks and never auto-trusts.** With no TTY (CI, a cloud agent, a
  ...
  > ⛔ **AMENDED 2026-08-28 — the contract survives; its subject changes from
  > registry to workspace.** Under
  > [`adr_hook_workspace_consent.md`](../adr/adr_hook_workspace_consent.md) the
  > thing that may not silently become trusted is an **unconsented workspace**,
  > not an untrusted registry. Everything else in this contract holds verbatim
  > and is *more* load-bearing than before, because consent is now the only
  > gate a clone can reach: never hang, never auto-consent, report the hook
  > as `gated` with cause `workspace-not-consented` (the state token the
  > registry cause it replaces already carried), exit **0** for the install as
  > a whole. **Nothing non-interactive ever writes a record** — the
  > write seam is `{grim hook allow, grim add, an accepted prompt}` and nothing
  > else, asserted by an acceptance test rather than by visibility.
  > The escape it names is `--trust-hooks` (the flag pair kept its name by
  > owner decision 2026-08-28), never `--allow-hooks`, which does not exist.
  > W5's amendment below is unchanged and still normative: interactive means
  > stdin **and** stderr are both terminals, and the prompt goes to stderr.
