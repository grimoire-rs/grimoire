...
lands the rework).
**Source of truth:** [`adr_description_companion.md`](adr/adr_description_companion.md).
...
- **Asset-heavy companion?** Prefer `grim fetch <ref> --description --out <dir>`
  to unpack the tree to disk in one call rather than holding every base64
  member in memory — cheaper for a README that references many images.
- **Replication caveat:** the companion rides a separate `__grimoire` tag, so
  a single-tag mirror (skopeo/oras copying one version tag) drops it; a
  full-repo sync carries it (see `publishing.md` → "Replication caveat").
