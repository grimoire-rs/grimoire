...
Vendor-coverage wave 2 adds eight client targets — the vendor-neutral
`agents` fallback, Google Antigravity, and six skills-only clients (Cline,
Droid, Goose, Warp, OpenClaw, Kilo) — plus a per-client
`[options.vendors.<name>].shared_skills` opt-in and support for the
`KIRO_HOME` and `GEMINI_CLI_HOME` directory overrides. None of it is a
breaking change. Three behaviour changes are visible enough on upgrade to
call out.

### Changed

- **Every client name now reserves its `<name>.*` metadata namespace, and
  seven names joined this release**: `antigravity`, `cline`, `droid`,
  `goose`, `warp`, `openclaw` and `kilo`. A published skill carrying
  `metadata.goose.foo` previously took the verbatim fast path and installed
  byte-identically; the key is now stripped, with a warning naming the key
  and the client. Reservation follows automatically from the client list —
  it is the same additive step every earlier client took (`codex`,
  `cursor`, `kiro`, `junie`, `gemini`, `zed`, `amp`).

  One consequence is counter-intuitive enough to state plainly, because it
  reads like a bug: these seven carry **empty field registries for every
  kind**, so such a key is dropped even when its *own* client is the
  target. Move client-specific data out of a client-named prefix, or
  rename the prefix — `vendor.foo` and anything else outside the reserved
  set still passes through untouched. As of this release the drop always
  warns: `amp`, `antigravity`, `codex`, `gemini` and `zed` used to drop
  own-namespace keys silently, which contradicted grim's own documented
  projection table, and now behave like everyone else.

- **Autodetect now sees the new clients, which can change a repeat
  install's reported status.** At project scope a workspace containing
  `.goose`, `.cline`, `.clinerules`, `.warp`, `.kilo`, `.kilocode` or
  `.factory` now picks that client up in the default target set; the
  global-scope markers are the matching home directories, plus
  `~/.openclaw` and Antigravity's `~/.gemini/config`. An existing install
  record therefore no longer covers every target, and the next `grim
  install` reports `installed`/`updated` where it previously reported
  `already-installed`. No schema or enum changed — those status literals
  already existed, and this is the intended meaning of supporting a new
  client: the alternative is a newly supported client staying invisible
  until you intervene. Set `[options].clients` (or pass `--client`) to pin
  a deterministic target set.

  One knock-on worth knowing: Antigravity's global root `~/.gemini/config`
  sits *inside* Gemini CLI's own `~/.gemini` marker, so a global
  Antigravity install makes **gemini** detected too, and uninstalling does
  not undo it — grim removes files, not the empty directories. Pass
  `--client` if you want only one of them.

- **Nothing detected no longer means "install into every client".** With
  no `--client`, no `[options].clients`, and no client marker present,
  `grim install` now targets a single vendor-neutral `agents` client that
  writes one copy into the cross-vendor `.agents/skills` pool. If the
  declared set holds nothing that client can install — only rules, agents
  and/or MCP servers — `grim install` and `grim add` exit **78** naming
  `--client` and `[options].clients` instead of writing a directory per
  vendor. `grim add` still records the declaration and the lock entry, so
  a follow-up `grim install --client <name>` completes without re-adding.

- **`$KIRO_HOME` and `$GEMINI_CLI_HOME` are now honored, and Zed's macOS
  root no longer follows `$XDG_CONFIG_HOME`.** The two variables have
  opposite shapes: `$KIRO_HOME` replaces `~/.kiro` outright, while
  `$GEMINI_CLI_HOME` replaces the *home directory*, so Gemini's root
  becomes `$GEMINI_CLI_HOME/.gemini`. If you already set either variable,
  artifacts grim installed before this release sit at a root grim no
  longer resolves — where your CLI was never reading them. The next
  `install`, `update` or `uninstall` reaps the stranded copy
  automatically, including an MCP entry spliced into the old settings
  file; a copy you edited by hand is preserved instead and reported in
  `retained` (or `abandoned_entries` for an MCP entry). Until one of those
  commands runs, `grim status` reports the artifact `not-installed`.

  The shared `$HOME/.agents/skills` pool deliberately does **not** follow
  `$GEMINI_CLI_HOME`: one physical tree serves every pool client under a
  single refcount, so it stays keyed on the real `$HOME`.

### Fixed

- **An install no longer clobbers a hand-authored file at a destination a
  release moved.** It now exits **65** with `reason:
  "untracked-destination"` (`forceable: true`) instead of silently
  overwriting at exit 0, restoring the promise grim already documented:
  grim does not delete files it did not create. Exactly one released path
  reached this — global Copilot rules migrating from
  `grim-home/.github/instructions/<name>` to
  `copilot-root/instructions/<name>`, shipped in 0.10.0. Re-run with
  `--force` to complete the migration and overwrite the file. No exit code
  was added or repurposed.

- `grim fetch --format json` and the MCP `grim_fetch` tool now populate
  the existing optional `warnings` array for `codex`, `gemini`, `zed`,
  `amp` and `antigravity`, where they previously emitted none. The field
  already documented "projection typo guards" as its content; nothing was
