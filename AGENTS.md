...
the Rust crate/package is `grimoire`. Shipping: full CLI (23 subcommands),
OCI registry pipeline, catalog publishing, MCP server, TUI, lifecycle hooks.
One binary crate, subsystem modules under `src/`, no stable *library* API —
the binary is the only consumer.
...
|---|---|---|
| `GRIM_HOME` | Root data directory (content store, catalog, global config, global-scope install state at `$GRIM_HOME/state/global.json`). Project-scope install state lives at `<workspace>/.grimoire/state.json`. Global-scope client output lands in vendor-native dirs — see subsystem-file-structure.md. **Hook arming lives here too** (`$GRIM_HOME/hooks/`: the dispatch table, the generated launcher, the root-key map, and the per-workspace consent records under `consent/`) and it is load-bearing, not incidental: a relative `GRIM_HOME`, or one nested inside the workspace, makes an armable file repo-resident and grim therefore **refuses to arm** rather than warning — causes `grim-home-relative` / `grim-home-in-workspace` | `~/.grimoire` |
...
| `GRIM_RATE_TOKEN` | Forge API token for `grim rate`, first rung of its **own** credential ladder (env → `gh`/`glab` stored credential → refuse with 80). Deliberately not the announce token: narrower scope, separate rotation. Never logged | (unset) |
| `GRIM_RATING_HOST` | Forge host `grim rate` contacts (GHES / self-managed GitLab). Per-machine by design — an index carries no host at all, so index-fetched content has no path to a credential destination. Empty ⇒ unset. Names a host, not a forge: it applies to whichever provider the catalog row declared. A loopback host (`localhost`, `127.0.0.1`, `[::1]`, any port) is contacted over plain HTTP; every other host over HTTPS | (unset) |
...
| *(no variable)* | **Hooks are deliberately unreachable from the environment.** Neither the `[options.experimental] hooks` feature flag nor the **workspace consent record** (`$GRIM_HOME/hooks/consent/<workspace-key>.json`) has an env form: there is no `GRIM_HOOK_CONSENT`, and no variable naming or relocating the record's path. `GRIM_ALLOW_HOOKS` **does not exist** either — removed by owner decision 2026-08-17 (commit `24a14bb`, withdrawing C-026) and must not be re-added. A repository routinely carries its own environment (`.envrc`, `.mise.toml`, devcontainer `containerEnv`, CI `variables:`), so an env-settable consent gate would let a repo consent on a cloner's behalf, and an env-settable record *path* would let it supply a pre-consented file — both CWE-426, both the same defect as the withdrawn committed-registration shape. The only per-invocation answers are the `--trust-hooks` / `--no-trust-hooks` **flag pair** on `install`/`update`/`add`, which cannot be set by a file — which is why the pair is allowed to outrank the record in both directions (owner decision 2026-08-28). Do not "helpfully" add a row here | n/a |
