...
| *(no variable)* | **Hooks are deliberately unreachable from the environment.** Neither the `[options.experimental] hooks` feature flag nor the per-registry `trust_hooks` grant has an env form, and `GRIM_ALLOW_HOOKS` **does not exist** — it was removed by owner decision 2026-08-17 (commit `24a14bb`, withdrawing C-026) and must not be re-added. A repository routinely carries its own environment (`.envrc`, `.mise.toml`, devcontainer `containerEnv`, CI `variables:`), so an env-settable arming gate would let a repo grant itself code execution on a cloner's machine (CWE-426). The only per-invocation escape is the `--allow-hooks` **flag** on `install`/`update`/`add`, which cannot be set by a file. Do not "helpfully" add a row here | n/a |
