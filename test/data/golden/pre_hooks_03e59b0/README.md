...
1. A reachable registry at **`localhost:5000`** — the host string is embedded in
   every `pinned = …` value in both golden locks and in both recorded
   `declaration_hash` values. **If the session registry host differs, skip
   rather than normalize:** rewriting the host would also rewrite the bytes
   under test. `test_golden_pre_hooks.py` carries that skip as a `pytestmark`.

   > **This is not the acceptance suite's default, and the note that said so
   > was wrong.** `test/conftest.py` starts its throwaway `registry:2` on a
   > **dynamic non-5000 port** precisely because 5000 is in grim's built-in
   > plain-HTTP allowlist, which would make every `insecure = true` test
   > unfalsifiable. So on a stock checkout this fixture **skips**, and the
   > C-015 evidence is checked only when a registry already answers on 5000.
   > Removing that caveat means regenerating the fixture against a
   > host-neutral reference — which must not be done from a tree that knows
   > about hooks (see "Never regenerate" above).
