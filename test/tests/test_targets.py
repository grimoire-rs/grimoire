...
def test_install_claude_and_copilot_layouts(
    ...
    # The file starts with YAML frontmatter whose first key is applyTo (the
    # paths→applyTo transform); the provenance comment follows the closing
    # --- fence.
    assert body.startswith("---\napplyTo:"), (
        f"copilot instructions must start with applyTo frontmatter, got: {body!r}"
    )
    assert "<!-- generated by grim from " in body, (
        f"copilot instructions must carry a provenance comment: {body!r}"
    )
    assert "edits will be overwritten -->" in body, (
        f"provenance comment must carry the overwrite warning: {body!r}"
    )
...
def test_editing_generated_copilot_file_is_detected_as_drift(
    ...
    restored = instr.read_text()
    # The regenerated file starts with applyTo frontmatter (paths→applyTo
    # transform) and carries the provenance comment after the closing fence.
    assert restored.startswith("---\napplyTo:"), (
        f"forced install must restore the applyTo frontmatter: {restored!r}"
    )
    assert "<!-- generated by grim from " in restored, (
        f"forced install must restore the provenance comment: {restored!r}"
    )
