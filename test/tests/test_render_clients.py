...
def test_opencode_scoped_rule_states_its_scope_in_the_body(
    grim_at, project_dir: Path, registry: str, unique_repo: str
) -> None:
    """OpenCode cannot express ``paths:``, so the scope is restated as prose in
    the body — otherwise the consumer gets a rule authored for ``**/*.rs`` that
    silently claims to apply to everything. The author is still warned that
    prose replaced a native key."""
    _push_rule_paths_only(unique_repo)
    write_config(project_dir, rules={"rust-style": f"{registry}/{unique_repo}/rust-style:v1"})
    runner = grim_at(project_dir)
    runner.run("lock", check=False)

    result = runner.run("install", "--client", "opencode", format="json", log_level="warn")
    rows = json.loads(result.stdout)["items"]
    assert all(r["status"] in ("installed", "unchanged") for r in rows), rows

    text = (project_dir / ".opencode/rules/rust-style.md").read_text()
    assert "> Applies only when working on files matching `**/*.rs`." in text, (
        f"the dropped scope must be restated as prose:\n{text}"
    )
    # Provenance first, then the notice, then the author's body.
    assert text.index("generated by grim") < text.index("Applies only") < text.index("Use 4 spaces."), (
        f"notice must sit between provenance and body:\n{text}"
    )
    assert "path scoping" in result.stderr, (
        f"dropping `paths` must still be warned, not silent; got: {result.stderr!r}"
    )


def test_opencode_unscoped_rule_gets_no_scope_notice(
    grim_at, project_dir: Path, registry: str, unique_repo: str
) -> None:
    """No ``paths:`` means nothing was dropped — no notice, no warning."""
    make_artifact(
        f"{unique_repo}/plain-style",
        "rule",
        {"plain-style.md": "# Plain\nAlways applies.\n"},
        tag="v1",
    )
    write_config(project_dir, rules={"plain-style": f"{registry}/{unique_repo}/plain-style:v1"})
    runner = grim_at(project_dir)
    runner.run("lock", check=False)

    result = runner.run("install", "--client", "opencode", format="json", log_level="warn")
    text = (project_dir / ".opencode/rules/plain-style.md").read_text()
    assert "Applies only" not in text, f"nothing was scoped: {text!r}"
    assert "path scoping" not in result.stderr, (
        f"an unscoped rule drops nothing; got: {result.stderr!r}"
    )
