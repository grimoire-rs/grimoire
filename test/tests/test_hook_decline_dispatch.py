...
def test_a_declined_mutator_is_not_dispatched_p1(
    ...
    runner.run("lock")
    install = runner.run("install", "--allow-hooks")
...
def test_an_artifact_whose_every_entry_is_declined_reads_not_armed_on_both_surfaces_p1(
    ...
    Trust is granted the durable way — `trust_hooks = true` in the **global**
    config — rather than with `--allow-hooks`, because a per-invocation grant is
    never persisted and the config-derived verdict then reports
    `registry-not-trusted` instead, which outranks this one (it is the actionable
    cause after a plain re-install). That is WP-R's F-2 residual, not this fix's.
...
def test_a_reserved_binding_name_is_refused_before_it_materializes_p2(
    ...
    *manifest* `name` of `bin`/`dispatch.json`/`payload`, but its only caller is
    ...
    install = grim.run("install", "--global", "--allow-hooks")
