...
| `config registry fields` | `{key, type, title, description}` — `key` is the short field name (`oci`, `index`, `default`, `include`, `exclude`, `insecure`, `trust_hooks`), deliberately diverging from `config list`'s dotted `registry.<alias>.<field>` keys; no `value`/`set`/`default`, since a field pattern (not a resolved alias) has no runtime value | — |
...
{
  "client": "claude",
  "cause": "registry-not-trusted",
  "message": "this artifact's registry is not trusted for hooks; set trust_hooks = true on its [[registries]] entry in your global config, then re-run grim install. An entry in either scope declaring trust_hooks = false overrides every grant, including the implicit one a namespaced global entry carries",
  "transient": false
}
```
...
| `gated` | `registry-not-trusted` | The artifact's registry carries no [`trust_hooks`](./configuration.md#registry-trust-hooks) grant. |
