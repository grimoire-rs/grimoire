...
| `hook list` | `{artifact, id, tier, events, state, arming}` — one object per `[[hooks]]` entry, not per artifact: `artifact` is the config binding name and `id` the entry's id, which together form the `<artifact>/<id>` identity the dispatch table and audit trail use. `events` is every moment the entry names (its canonical event plus any native `<vendor>.event`) and is never `[]` — `grim build` rejects an entry naming no moment. `arming` is a **nested** array of the same `{client, cause, message, transient}` objects `status` carries, sorted by client, `[]` meaning armed on every configured client rather than "unknown". An artifact whose payload was never materialized (a declined hook is skipped before its blob is fetched) contributes no items and one stderr warning — see [Hook reporting](./stability.md#limitations-hook-reporting) | `tier`: `observer`, `gatekeeper`, `mutator`; `state`: same tokens as `status` |
...
`kind` is one of `skill`, `rule`, `agent`, `bundle`, `mcp`, `hook` for every
...
| `config registry show` | `{alias, oci, index, include, exclude, default, insecure}` — both locator keys present, exactly one non-null; `include`/`exclude` always-present arrays, `[]` when unfiltered; `insecure` is the authored opt-in, not the effective transport. These six are the whole per-registry surface — hook arming is a [workspace](./configuration.md#workspace-consent) question, not a registry field, and nothing about it is reported here | — |
...
### Hook arming {#hook-arming}

A [hook](./artifacts.md#hooks) row on `status` and every row on `hook list`
carries an `arming` array beside `state`. Each element is one client's
verdict:

```json
{
  "client": "claude",
  "cause": "workspace-not-consented",
  "message": "this workspace has not consented to arming hooks; run grim hook allow to record consent for it, then re-run grim install. Consent is per workspace and machine-local — cloning a repository never carries it",
  "transient": false
}
```

**Branch on `cause`, never on `message`.** `cause` is a closed, kebab-case
enum and part of the frozen report contract; `message` is human-facing text
carrying no compatibility promise (see [Unstable
surfaces](./stability.md#unstable)). `transient` is `true` only when
re-running unchanged may succeed — today only a dispatch lock held by a
concurrent `grim install` — and `false` on every `grim status` row, since a
transient cause is filtered before it reaches that report.
...
| `state` | `cause` | Meaning |
|---------|---------|---------|
| `gated` | `feature-flag-off` | `[options.experimental] hooks` is not enabled for this scope. |
| `gated` | `client-has-no-hook-surface` | This client has no hook surface at this scope at all — the [declined](./clients.md#gap-hooks) path, not a failure. |
| `untrusted` | `client-trust-pending` | Armed, but the client has not approved the registration yet. Codex requires an interactive `/hooks` approval and skips an unapproved hook silently. |
| `gated` | `workspace-not-consented` | This workspace carries no [consent record](./configuration.md#workspace-consent). Cloning a repository never carries one; `grim hook allow` writes it. |
| `gated` | `consent-drifted` | A record exists, but the workspace now declares a hook it does not cover — a new binding, or one from a new repository. The `message` names the new entry. A version bump of an already-consented hook is deliberately **not** drift. |
| `not-armed` | `insecure-transport` | The pinned registry host is not loopback and is reached over plain HTTP, where the first resolution that produces the digest pin is itself attacker-influenceable. Independent of consent, and answerable only by `--trust-hooks`. |
| `not-armed` | `grim-home-relative` | `GRIM_HOME` is a relative path, so the dispatch table would resolve inside the repository. |
| `not-armed` | `grim-home-in-workspace` | `GRIM_HOME` resolves inside the workspace, which would make an armable file repo-resident. |
| `not-armed` | `launcher-path-control-character` | The resolved launcher path holds a newline or another control character, which no vendor's shell round-trip quotes correctly. |
| `not-armed` | `not-registered` | grim registered nothing for this client, so nothing runs — usually the entry's tier at that event, or its matcher. Re-run `grim install` to see the reason it reports. |
| `not-armed` | `dispatch-lock-held` | Another `grim install` holds the dispatch lock. **The only `transient: true` cause.** |

New causes may be added in a minor release under the [additive
policy](#null-policy) — treat an unrecognized `cause` as "not armed, reason
unknown to this consumer" and show `message`, rather than erroring.
