...
2. **Per-registry trust** — the artifact's registry must be trusted for
   hooks via [`trust_hooks`](./configuration.md#registry-trust-hooks). A
   `false` anywhere wins over every grant, and a project-scope entry can
   only ever restrict.

Until both pass, [`grim status`](./commands.md#status) reports the hook as
`gated` and names the cause; the exit code stays `0`, because a gate doing
its job is not a failure. For a run with no terminal to prompt on — CI, a
cloud agent — [`grim install --allow-hooks`](./commands.md#install) is the
per-invocation escape, and there is deliberately no environment variable
...
| `[hooks]` | no | name → ref table | [Hook](#hooks) members. A bundle carrying one is disclosed on install, and the member's **own** registry must be trusted for hooks — a trusted bundle does not launder trust to a member pinned elsewhere |
...
bundle member must be a skill, rule, agent, or hook.
