...
| `GRIM_RATING_HOST` | Forge host [`grim rate`](./ratings.md#voting-against-a-private-instance) contacts, for GitHub Enterprise Server or a self-managed GitLab. Per-machine, not per-project: an index never supplies it, so no index-fetched content can redirect a credential. Empty or whitespace-only counts as unset. Applies to whichever provider the catalog row declared — set it per shell or per instance rather than globally on a machine that browses both a public and a private index. A loopback host (`localhost`, `127.0.0.1`, `[::1]`, on any port) is contacted over plain HTTP so a local fake forge is reachable; every other host is contacted over HTTPS. | unset |
