...
`grim schema --kind <config|publish|lock>` prints a [JSON
...
published so tooling can validate or introspect it). Each schema is
generated from grim's own parser, so it accepts exactly what grim accepts.
