...
endpoint exposes a per-artifact download counter, and the two most widely
used registries — GHCR and the GitLab registry — publish none in any API,
so on most indexes every row carries `null` here. A client must render
absence as "unknown", never as `0`: a `total` of `0` is a real measurement
that the producer took and nobody has pulled the artifact.

`as_of` is when the producer **read** the counters, and it is `null` when
the sidecar published no stamp. It is deliberately not the sidecar's
document-level `generated_at`: a rebuild that only recomputes ratings
republishes a carried-forward count whose stamp is older than the document
around it. Show it beside the figure — a pull count is a measurement with a
