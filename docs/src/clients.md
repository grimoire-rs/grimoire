...
Hooks are the one kind where "can host it" is a question about *arming*, not
about writing a file. A hook only does something once the client is registered
to run it on a named lifecycle event, at a tier the client honours — so the
Hook column reports whether grim can register a hook there at all, and every
`✗` in it means grim writes and registers nothing (see
[Hooks: three clients in v1](#gap-hooks)).
...
Read the Hook column the same way, with two things it deliberately does not
say. It is **scope-blind** — [Codex] and [Copilot] arm hooks at global scope
only, exactly as the Rule column does not encode [Junie]'s missing global
surface. And it is about capability, not consent: installing a hook also
requires the explicit opt-in and the workspace consent that gate every hook,
...
| Client | Skill | Rule | Agent | MCP | Hook |
|--------|-------|------|-------|-----|------|
...
Fifteen of the eighteen rows decline hooks, and the reason is **not the same
one** in every row. Three different situations produce the same `✗`, and only
one of them is grim's to change.

*No upstream surface at all* — [Zed] and [Warp]. [Zed]'s agent runs over ACP
with no lifecycle-hook mechanism, and [Warp] documents none either. There is
nothing to register with, so this decline is not a scheduling choice: it cannot
be reversed by a grim release alone, only by the vendor shipping a surface
first.

*Not a client* — the vendor-neutral `agents` target is a shared directory, not
a product, so no process reads a hook registration placed there. Same reason it
declines rules, agents and MCP.

*A surface exists upstream, and grim has not wired it yet* — the remaining
twelve: [OpenCode], [Cursor], [Kiro], [Junie], [Gemini], [Amp], [Antigravity],
[Cline], [Droid], [Goose], [OpenClaw] and [Kilo]. Each documents its own hooks
mechanism; grim declines by **schedule**, and adding support later is additive.
They split into three shapes, which is why they are not all equally close:
[Cursor], [Gemini], [Droid], [Antigravity] and [Junie] splice shell-command
hooks into a shared config file; [Kiro], [Goose] and [Cline] own a hooks file
or directory of their own; [OpenCode], [Amp], [Kilo] and [OpenClaw] take a
JavaScript or TypeScript plugin module, which grim would have to generate
rather than register. The first two shapes reuse machinery grim already has,
the third does not.

The three that do arm — [Claude], [Codex] and [Copilot] — were chosen because
they read the same wire contract, so one canonical hook projects to all three
without per-client semantics: [Claude] splices its `settings.json`
(`.claude/settings.local.json` at project scope), while [Codex] and [Copilot]
each own a `hooks.json` grim writes and deletes outright.

A `✓` here is a statement about the client, not a promise that every hook you
write installs into it. A single hook can still be refused on its own merits —
for instance a hook that rewrites a tool's input is refused when its matcher
selects every tool, because that includes tools that run shell commands. Those
refusals depend on the hook, so they are reported per hook at install time
rather than in this column.

### Codex and Copilot: hooks are global-scope only {#gap-hooks-scope}

[Codex] and [Copilot] register hooks at **global scope only**. Both have a
working project surface upstream — `.codex/hooks.json` and
`.github/hooks/*.json` — and both of those are ordinary *tracked* repository
files. A registration there would put something that causes execution inside a
repository, where cloning an untrusted repo would arm it, and it would have to
name a launcher path assembled from the environment that repository can itself
influence. grim therefore warns, skips, and records no output for a
project-scope hook on either client; install it with `--global`.

[Claude] is not affected, because its project registration lands in
`.claude/settings.local.json` — a file [Claude] itself treats as local,
per-developer state rather than shared repository content.

### Codex: hooks need a POSIX login shell {#gap-codex-shell}

[Codex] runs a hook command through **the user's own login shell**
(`$SHELL -lc`), where [Claude] uses `/bin/sh` and [Copilot] uses `bash`. grim
registers a POSIX `sh` one-liner, so a user whose login shell is `fish` or
`nushell` gets a hook that **installs, reports armed, and never runs**: the
registration contains `s=$?` and a `case … esac`, neither of which parses in
those shells, so the command fails before it reaches grim's launcher.

The failure is silent in both directions. [Codex] does not block a tool call on
a non-zero hook exit, so nothing surfaces to the user; and `grim status` reports
the hook `installed`, because grim's own work genuinely is complete — the
registration was written exactly as intended.

**This matters most for a `gatekeeper`,** which is the one configuration where a
reader is told a guardrail is armed while it can never fire. If your login shell
is not POSIX-compatible, either run [Codex] with `SHELL=/bin/sh`, or do not rely
on a [Codex] hook as a guardrail. A shell-neutral registration is the intended
fix and is tracked upstream of this page; until it ships, this constraint is the
contract.

[Claude] and [Copilot] are unaffected — grim picks their interpreter, not the
user's.

### Copilot: no blocking verdict after a tool runs {#gap-copilot-hooks}

[Copilot]'s `◐` is one specific gap: it accepts no blocking verdict on the
*after a tool ran* event, where [Claude] and [Codex] both do. A hook whose
response is meant to allow or deny is therefore refused for [Copilot] on that
event — warn, skip, no registration — while every other event and tier
[Copilot] admits arms normally.

That gap is a property of [Copilot]'s own event table, and it holds for every
matcher form — one exact tool name and match-all alike. That is precisely what
makes it degrade the cell: a refusal that depends on which tools the hook
matches is a per-hook fact, reported with that hook, and never a client-wide
`◐`.

### An `ask` verdict is `PreToolUse`-only {#gap-hooks-ask}

A `gatekeeper` may return four verdicts — `allow`, `deny`, `ask`, or none — but
**`ask` is only expressible before a tool runs.** No v1 client has a field for
"escalate to the user" on the *after a tool ran* or *turn stopped* events; those
events accept a block and an allow and nothing else.

A hook that answers `ask` at one of those moments is **reported to the client as
a block**, with the hook's own reason attached, and grim logs that it did so. The
substitution is deliberately more restrictive than the author asked: `ask` means
"let the user decide", and where that cannot be said, blocking *with the reason
visible* tells the user more than silence — which every client reads as allow.

This is not a per-client gap and it does not degrade any cell: it holds for all
three v1 clients, because it is a property of what the events themselves can
carry. Authors targeting those events should return `deny` with a reason and mean
it.
...
The scope itself is not lost. Where a rule declares `paths`, grim writes it into
the rule body as one line of prose ahead of your text:

```markdown
<!-- generated by grim from … -->
...
That is a statement the model reads and gates on, not a mechanism the client
enforces — weaker than a native scoping key, which is why the install still
warns. It costs one line, it is removed with the artifact, and it stops a rule
authored for one file type being applied to everything.
...
rule's `paths` has nowhere to land and the rule loads unconditionally. As for
[OpenCode], grim restates the dropped scope as one line of prose in the rule
body so the model still gates on it — which matters more here, where several
rules end up concatenated into a single context.
