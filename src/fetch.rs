...
pub async fn fetch_artifact(
    ...
    // offline-blocked (81), matching the sibling digest-only/describe paths and
    // the documented `fetch`/`fetch --description` contract — a bare `Query`
    // would fold that into a plain not-found (79). The MCP `grim_render` tool
    // rides this same caller (via `mcp::render`), so it inherits the identical
    // offline-blocked semantics. Online, this write-throughs the tag pin like
    // every other resolve.
...
pub async fn describe_artifact(
    ...
    Ok(DescribeReport {
        ...
        has_description,
        summary: get(crate::oci::annotations::SUMMARY_ANNOTATION),
...
mod tests {
    ...
    #[tokio::test]
    async fn fetch_description_non_companion_manifest_classifies_data_error() {
        // W6: the reserved `__grimoire` tag resolves to a real (skill) manifest
        // rather than a `com.grimoire.kind: desc` companion — malformed external
        // data (65), the same tier as an empty companion, NOT a not-found.
        let tmp = tempfile::tempdir().unwrap();
        let reg = MemoryRegistry::new();
        // A genuine skill manifest published AT the reserved companion tag.
        publish(
            &reg,
            "test.registry/acme/skills/demo:__grimoire",
            "skill",
            &tar_of(&[("demo/SKILL.md", b"# x\n")]),
        )
        .await;
        let (scope, access) = scope_and_access(&reg, tmp.path(), tmp.path());

        let err = fetch_description(&scope, &access, "test.registry/acme/skills/demo:latest", None)
            .await
            .expect_err("a non-companion manifest at the reserved tag must error");
        assert_eq!(
            crate::error::classify_error(&err),
            crate::cli::exit_code::ExitCode::DataError,
            "a __grimoire tag that is not a desc companion is a data error (65)"
        );
    }

    #[tokio::test]
    async fn fetch_description_empty_companion_classifies_data_error() {
        // W6: a desc companion whose tar layer carries no entries is malformed
        // external data (65) — the unpack guard rejects an empty archive.
        let tmp = tempfile::tempdir().unwrap();
        let reg = MemoryRegistry::new();
        // A desc-kind manifest at the reserved tag whose tar has no members.
        publish(&reg, "test.registry/acme/skills/demo:__grimoire", "desc", &tar_of(&[])).await;
        let (scope, access) = scope_and_access(&reg, tmp.path(), tmp.path());

        let err = fetch_description(&scope, &access, "test.registry/acme/skills/demo:latest", None)
            .await
            .expect_err("an empty companion must error");
        assert_eq!(
            crate::error::classify_error(&err),
            crate::cli::exit_code::ExitCode::DataError,
            "an empty companion tar is a data error (65)"
        );
    }
