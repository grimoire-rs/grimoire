...
/// `[skills]`/`[rules]`/`[agents]`/`[hooks]` tables are the members — with
...
pub struct BundleSource {
    ...
    /// nothing — the install seam owns that decision (feature flag, then
    /// `trust_hooks`, then the registrar), and it keys the trust predicate on
    /// the **member's own** `LockedSource`, never on the bundle's: a bundle
    /// from registry A may legitimately pin a member from registry B, and
    /// approving A must not silently grant B.
    ///
