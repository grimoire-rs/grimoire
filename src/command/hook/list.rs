...
//! this file re-derives the feature flag, the per-registry `trust_hooks`
//! decision, the `$GRIM_HOME` refusal, or the dispatch table's arming
//! authority. A second derivation of those gates is exactly how the two
...
pub async fn run(ctx: &Context, _args: &ListArgs) -> anyhow::Result<(HookListReport, ExitCode)> {
    ...
    let inputs = status::HookArmingInputs::resolve(ctx, &scope, &target)?;
