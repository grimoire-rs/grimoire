...
pub fn read_bundle_members(
    ...
    // unchanged bundle's layer digest cannot move. The same holds for `[mcp]`.
...
mod tests {
    ...
    fn read_bundle_members_covers_every_member_table() {
        ...
        // agent members. `[hooks]` and `[mcp]` were the same shape one step
        // earlier: `[hooks]` was parsed but dropped here, and `[mcp]` did not
        // exist at all, so a descriptor could not be bundled even though every
        // consumer treats an mcp member as first-class.
        let tmp = tempfile::tempdir().unwrap();
        ...
        assert_eq!(
            ...
            "every member table maps onto the wire, agents, hooks and mcp included"
        );
    }
