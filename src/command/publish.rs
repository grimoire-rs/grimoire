...
use crate::catalog::forge::ForkPolicy;
...
/// The `[announce] fork` value: an explicit [`ForkPolicy`] string, or the
/// legacy boolean toggle (`true` → [`ForkPolicy::Auto`], `false` →
/// [`ForkPolicy::Never`]) that predates the policy.
///
/// `Deserialize` is hand-written (rather than `#[serde(untagged)]`) for the
/// same reason as [`EntryDescription`]: a misspelled policy must surface
/// [`ForkPolicy`]'s own error — `unknown variant 'alwyas', expected one of
/// 'never', 'auto', 'always'`, with the variant list generated by its derive —
/// instead of serde's untagged catch-all (`data did not match any variant`).
/// The `#[serde(untagged)]` attribute stays only to drive the `JsonSchema`
/// `anyOf` shape (schemars reads it; the manual impl overrides how bytes are
/// parsed).
#[derive(Debug, Clone, Copy, JsonSchema)]
#[serde(untagged)]
pub enum ForkSetting {
    /// Legacy boolean: `true` = auto (default), `false` = never.
    Toggle(bool),
    /// Explicit policy: `"never"`, `"auto"`, or `"always"`.
    Policy(ForkPolicy),
}

impl<'de> Deserialize<'de> for ForkSetting {
    fn deserialize<D>(deserializer: D) -> Result<Self, D::Error>
    where
        D: serde::Deserializer<'de>,
    {
        struct ForkSettingVisitor;

        impl serde::de::Visitor<'_> for ForkSettingVisitor {
            type Value = ForkSetting;

            fn expecting(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                f.write_str("a boolean or a fork policy string")
            }
            ...
            where
                E: serde::de::Error,
            {
                // Forward to `ForkPolicy` so its derived `unknown variant`
                // error — which lists every accepted value — reaches the user.
                ForkPolicy::deserialize(serde::de::IntoDeserializer::into_deserializer(value)).map(ForkSetting::Policy)
            }
        }

        deserializer.deserialize_any(ForkSettingVisitor)
    }
}

impl ForkSetting {
    /// Resolve the setting (legacy bool or explicit string) to a canonical
    /// [`ForkPolicy`].
    pub fn policy(self) -> ForkPolicy {
        match self {
            ForkSetting::Toggle(true) => ForkPolicy::Auto,
            ForkSetting::Toggle(false) => ForkPolicy::Never,
            ForkSetting::Policy(p) => p,
        }
    }
}
...
pub struct AnnounceSpec {
    ...
    /// How to fork the index before announcing. Accepts the legacy boolean
    /// (`true` = auto, `false` = never) or an explicit policy string
    /// `"never" | "auto" | "always"`. Default (unset): `auto`.
    ///
    /// - `auto` (or `true`): fork only when the authenticated forge token has
    ///   no push access to the index, then open a cross-repository PR/MR; with
    ///   push access, push to the upstream index directly.
    /// - `never` (or `false`): always push to the upstream index directly.
    /// - `always`: fork and open a cross-repository PR/MR even when the token
    ///   has push access (prefer PR review / dogfood the contributor path).
    ///
    /// A fork is never attempted without a forge API token.
    pub fork: Option<ForkSetting>,
}
...
async fn run_announce(
    ...
    // Resolve the fork policy (`[announce] fork`): unset/`true`/`"auto"` →
    // fork only when the token lacks push access; `false`/`"never"` → always
    // push upstream; `"always"` → fork even with push access.
    let fork = spec.and_then(|s| s.fork).map_or(ForkPolicy::Auto, ForkSetting::policy);
...
mod tests {
    ...
    #[test]
    fn fork_setting_resolves_legacy_bool_and_policy_string() {
        #[derive(serde::Deserialize)]
        struct W {
            fork: ForkSetting,
        }
        let policy = |s: &str| toml::from_str::<W>(s).expect("parse").fork.policy();
        // Legacy boolean form.
        assert_eq!(policy("fork = true"), ForkPolicy::Auto);
        assert_eq!(policy("fork = false"), ForkPolicy::Never);
        // Explicit policy string.
        assert_eq!(policy(r#"fork = "auto""#), ForkPolicy::Auto);
        assert_eq!(policy(r#"fork = "never""#), ForkPolicy::Never);
        assert_eq!(policy(r#"fork = "always""#), ForkPolicy::Always);
        // Unset (Option::None) falls back to Auto at the resolution site.
        assert_eq!(
            None::<ForkSetting>.map_or(ForkPolicy::Auto, ForkSetting::policy),
            ForkPolicy::Auto
        );
    }

    /// A misspelled policy must name every accepted value. The hand-written
    /// `Deserialize` exists for this: `#[serde(untagged)]` would report the
    /// useless `data did not match any variant` instead.
    #[test]
    fn fork_setting_rejects_an_unknown_policy() {
        #[derive(Debug, serde::Deserialize)]
        struct W {
            #[allow(dead_code)]
            fork: ForkSetting,
        }
        let error = toml::from_str::<W>(r#"fork = "alwyas""#)
            .expect_err("a misspelled policy must not parse")
            .to_string();
        assert!(
            error.contains("alwyas") && error.contains("never") && error.contains("auto") && error.contains("always"),
            "the error must name the offending value and every accepted one, got: {error}"
        );
        // A value of neither shape reports the expected types, not a variant list.
        let type_error = toml::from_str::<W>("fork = 3")
            .expect_err("an integer is neither a boolean nor a policy string")
            .to_string();
        assert!(
            type_error.contains("boolean") && type_error.contains("fork policy string"),
            "the error must state both accepted shapes, got: {type_error}"
        );
    }
