...
pub struct FetchArgs {
    ...
    /// listing) instead of the index document. UTF-8 text prints verbatim;
    /// binary content is returned base64-encoded (JSON `encoding: base64`).
    #[arg(long)]
...
pub async fn run(ctx: &Context, args: &FetchArgs, format: OutputFormat) -> anyhow::Result<(FetchCliReport, ExitCode)> {
    ...
    if args.description && args.vendor.is_some() {
        return Err(crate::command::config_usage(
            "--vendor projects a per-client artifact document; it does not apply to the --description companion",
        ));
    }
