...
impl Printable for DescribeCliReport {
    fn print_plain(&self, w: &mut impl Write) -> io::Result<()> {
        ...
        let rows: Vec<Vec<String>> = vec![
            ...
            vec!["authors".into(), opt(&d.authors)],
            vec!["vendor".into(), opt(&d.vendor)],
            vec!["url".into(), opt(&d.url)],
            vec!["documentation".into(), opt(&d.documentation)],
            vec!["compatibility".into(), opt(&d.compatibility)],
            vec!["support.issues".into(), opt(&d.support.issues)],
            vec!["support.chat".into(), opt(&d.support.chat)],
            vec!["support.contact".into(), opt(&d.support.contact)],
            vec!["support.security".into(), opt(&d.support.security)],
...
mod tests {
    ...
    fn report() -> DescribeCliReport {
        ...
        DescribeCliReport(DescribeReport {
            ...
            revision: None,
            authors: Some("Platform Team".to_string()),
            vendor: Some("acme".to_string()),
            url: Some("https://acme.example/code-review".to_string()),
            documentation: Some("https://github.com/acme/code-review#readme".to_string()),
            compatibility: Some("claude>=2".to_string()),
            support: crate::oci::description::SupportLinks {
                issues: Some("https://github.com/acme/code-review/issues".to_string()),
                ..Default::default()
            },
            created: None,
