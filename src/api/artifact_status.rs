...
pub enum ArtifactStatus {
    ...
    /// the experimental feature flag is off, the source registry has not been
    /// granted `trust_hooks`, or the client has no hook surface at all
...
pub enum HookArmingCause {
    ...
    FeatureFlagOff,
    /// The artifact's source registry carries no `trust_hooks = true`
    /// (C-022). Per-registry consent, expressed in TOML.
    RegistryNotTrusted,
...
impl HookArmingCause {
    ...
    pub fn state(self) -> ArtifactStatus {
        match self {
            ...
            Self::FeatureFlagOff | Self::RegistryNotTrusted | Self::ClientHasNoHookSurface => ArtifactStatus::Gated,
        }
    }
    ...
    pub fn message(self) -> &'static str {
        match self {
            ...
            // Covers BOTH trust decisions this cause collapses (see
            // `command::status`'s `artifact_cause`): never granted, and
            // explicitly opted out. The old wording said "add trust_hooks =
            // true", which is wrong advice for a user whose entry already says
            // `false` — and it pointed only at the global config, while a
            // project entry may carry the opt-out that is actually in force.
            Self::RegistryNotTrusted => {
                "this artifact's registry is not trusted for hooks; set trust_hooks = true on its \
                 [[registries]] entry in your global config, then re-run grim install. An entry in \
                 either scope declaring trust_hooks = false overrides every grant, including the \
                 implicit one a namespaced global entry carries"
            }
...
impl std::fmt::Display for HookArmingCause {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str(match self {
            ...
            Self::RegistryNotTrusted => "registry-not-trusted",
...
mod tests {
    ...
    fn every_actionable_cause_names_a_remedy() {
        for cause in HookArmingCause::ALL {
            ...
            assert!(
                message.contains("grim install") || message.contains("/hooks"),
    ...
    fn causes_map_to_their_documented_state_token() {
        ...
        assert_eq!(HookArmingCause::RegistryNotTrusted.state(), ArtifactStatus::Gated);
