...
async fn perform(
    ...
    // are `[options.experimental] hooks` in config (C-026's replacement,
    // config-only by owner decision) and the workspace's own consent record
    // (`grim hook allow`). Neither is something the TUI asks for or
...
async fn perform_local_dev(
    ...
    match kind {
        ...
        // recorded decision: a working-tree path would put an armable payload
        // inside a repository, which invariant I1 forbids outright). Same
        // refusal the CLI dev-install path gives, reached from the TUI's dev
        // action.
