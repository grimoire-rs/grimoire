...
/// Subdirectory of [`HOOKS_DIR`] holding one consent record per workspace.
///
/// A [`RESERVED_ARTIFACT_NAMES`](crate::oci::hook::RESERVED_ARTIFACT_NAMES)
/// entry, and the reason is sharper than for its siblings: a hook bound to the
/// name `consent` would materialize a payload tree exactly where the answers to
/// *"may hooks arm here?"* live, and `grim uninstall` of that hook would then
/// reap every workspace's consent in one command.
pub const CONSENT_DIR: &str = "consent";
...
/// `$GRIM_HOME/hooks/consent` — one record per consented workspace.
///
/// Under `hooks/` rather than `$GRIM_HOME/state/` deliberately: everything that
/// decides whether a hook arms lives in one directory, and the reserved-binding
/// machinery that keeps a hook from colliding with grim's own files already
/// guards this namespace. [`CONSENT_DIR`] is therefore a
/// [`RESERVED_ARTIFACT_NAMES`](crate::oci::hook::RESERVED_ARTIFACT_NAMES) entry,
/// and `every_grim_owned_name_under_hooks_is_a_reserved_binding_name` fails the
/// build if that is ever undone.
pub fn consent_dir(grim_home: &Path) -> PathBuf {
    hooks_dir(grim_home).join(CONSENT_DIR)
}

/// `$GRIM_HOME/hooks/consent/<workspace-key>.json`.
///
/// **One file per workspace, never one shared file.** Decision E records why a
/// whole-file rewrite is the wrong shape at this layer: a command operates on
/// exactly one scope, so regenerating a shared file would delete every other
/// workspace's answer and silently disarm unrelated repositories. Per-workspace
/// files remove the read-modify-write entirely — no lock, no cross-workspace
/// blast radius, and a corrupt record costs exactly one workspace.
///
/// The filename is only a **lookup index**; the record's own `workspace` field
/// is the identity that decides. See [`crate::hook::consent::evaluate`].
pub fn consent_path(grim_home: &Path, workspace: &Path) -> PathBuf {
    consent_dir(grim_home).join(format!("{}.json", workspace_key(workspace)))
}

/// Create `$GRIM_HOME/hooks/consent` with owner-only permissions.
///
/// Same contract as [`ensure_hooks_dir`], which it delegates to for the parent:
/// idempotent, and it narrows only a directory it freshly created.
pub fn ensure_consent_dir(grim_home: &Path) -> io::Result<PathBuf> {
    ensure_hooks_dir(grim_home)?;
    let dir = consent_dir(grim_home);
...
/// Widened from private for [`crate::hook::consent`]: the consent record and the
/// payload directory must key the same workspace the same way, and the only way
/// to guarantee that is one function rather than two spellings. (Same reason
/// WP-G widened `registry_resolve::normalize_locator` for `hook::trust`.)
///
/// Note what is **not** promised: the path is not canonicalized first. Two
/// spellings of one directory therefore key differently, which fails **safe** —
/// the record simply does not match and the workspace re-gates.
