...
/// metadata value.
pub const COPILOT_AGENT_FIELDS: &[KnownField] = &[KnownField {
    field: "tools",
    native: "tools",
...
const COPILOT_AGENT_OVERRIDES: &[&str] = &["tools"];
...
impl Vendor for CopilotVendor {
    ...
    fn agent_index(&self, parsed: &ParsedAgent, pinned: &str) -> Result<Option<RenderedDoc>, RenderError> {
        ...
        // emit natively, the common `tools` comma string becomes the YAML
        // list Copilot reads (a `copilot.tools` key overrides it), and
        // `model` is dropped — Copilot documents no model field.
...
mod tests {
    ...
    #[test]
    fn agent_index_emits_name_description_and_tools_list_drops_model() {
        ...
        assert!(!out.document.contains("model"), "Copilot documents no model field");
