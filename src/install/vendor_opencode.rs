...
impl Vendor for OpenCodeVendor {
    ...
    fn rule_index(
        ...
        // OpenCode has no per-file rule scoping (Degraded per
        // adr_vendor_wave_expansion §2): the `paths:` key has no native
        // target and is dropped. The scope itself is not lost — it is
        // restated in the body as prose so the model self-gates
        // (class-2 compensating render, adr_vendor_support_tiers). The
        // author still gets the warning: prose is weaker than a native
        // key, and only the author can decide whether that is enough.
        let mut document = provenance(pinned);
        if !parsed.frontmatter.paths.is_empty() {
            warnings.push(
                "path scoping ('paths:') is dropped for OpenCode: it has no per-file rule scoping, \
                 so the scope is rendered into the rule body as prose instead"
                    .to_string(),
            );
...
mod tests {
    ...
    /// Specify phase: assert `docs/src/content/docs/vendor-metadata.md` documents exactly
    ...
    #[test]
    fn rule_index_strips_frontmatter_and_renders_scope_as_prose() {
        let doc = "---\npaths: [\"**/*.rs\"]\n---\n# Rust Style\nbody\n";
        ...
        let out = OpenCodeVendor
            ...
            .unwrap()
            .unwrap();
        // Provenance, the compensating scope notice, then the author's body.
        assert_eq!(
            out.document,
        ...
        // A scoped rule still warns: `paths:` scoping is Degraded for
        // OpenCode (ADR §2), and prose is weaker than a native key.
    ...
    fn rule_index_unscoped_rule_is_warning_free() {
        // An unscoped rule (no `paths:`) drops nothing — no warning, and no
        // scope notice to render either.
