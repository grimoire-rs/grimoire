...
impl Vendor for OpenCodeVendor {
    ...
    fn rule_index(
        ...
        // adr_vendor_wave_expansion §2): a scoped rule installs with its
        // `paths:` dropped and loads unconditionally. Surface the loss so the
        // author knows the scope is not honored (mirrors Kiro's inert-scoping
        // warning).
        if !parsed.frontmatter.paths.is_empty() {
            warnings.push(
                "path scoping ('paths:') is dropped for OpenCode: it has no per-file rule scoping, \
                 so this rule loads unconditionally"
                    .to_string(),
            );
        }
...
mod tests {
    ...
    fn rule_index_strips_frontmatter_and_adds_provenance() {
        ...
        // A scoped rule warns: `paths:` scoping is Degraded (dropped) for
        // OpenCode — it has no per-file rule scoping (ADR §2).
    ...
    fn rule_index_unscoped_rule_is_warning_free() {
        // An unscoped rule (no `paths:`) drops nothing — no warning.
